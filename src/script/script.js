...
window.addEventListener("mousemove",({clientX:x, clientY:y})=>{
    ...
    v[0] = mouse.diff(v[0]);
    v[0].len = v[0].lenght();
    v[0] = v[0].mul(1/v[0].len).mul(maxMoveLenght*((v[0].len/screenMaxLenght)**2));
    v[1] = mouse.diff(v[1]);
    v[1].len = v[1].lenght();
    v[1] = v[1].mul(1/v[1].len).mul(maxMoveLenght*((v[1].len/screenMaxLenght)**2));
    //v[0] = v[0].mul(v[0].lenght()/screenMaxLenght)
    moveables[0].style.setProperty("--x", v[0].x+"px");
    moveables[0].style.setProperty("--y", v[0].y+"px");
    moveables[1].style.setProperty("--x", v[1].x+"px");
    moveables[1].style.setProperty("--y", v[1].y+"px");
    console.log(screenMaxLenght, v[0], v[1])
})
